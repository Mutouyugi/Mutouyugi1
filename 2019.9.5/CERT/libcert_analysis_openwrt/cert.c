...
int tX509_analysis(char *cert_path, cert_st **cert_verify_st)
{
	...
	switch(ver)  	
	{
		...
		break;
		default:
		memcpy((*cert_verify_st)->version , "unknow", 6);
		break;
	} 
	//serialnumber
	...
	issuer = X509_get_issuer_name(m_pX509);
 	for (i = 0; i < X509_NAME_entry_count(issuer); i++) 
	 {
		...
		switch(i_NID)
		{
			...
			case    48:       memcpy((*cert_verify_st)->issuer_name[i].NID,"E",1);             break;
			default:          memcpy((*cert_verify_st)->issuer_name[i].NID,"unknow",6);
		}
 	}
	//validity
	int j=0;
	ASN1_TIME *before_asn1_time ;
	ASN1_TIME *after_asn1_time ;
...
int main ()
{
    ...
    char*cerfile="/home/mutouyugi/Documents/CERT/1.cer";
